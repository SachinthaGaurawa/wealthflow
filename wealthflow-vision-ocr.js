...
(function () {
    ...
    function ocrBase64(base64, opts) {
        ...
        var p = _wfFetchT(_apiBase() + '/vision', {
    ...
    function fileToImages(file) {
        ...
        return new Promise(function (resolve) {
            try {
                ...
                r.onload = function () {
                    var image = new Image();
                    // No try/catch of its own previously: a throw anywhere in
                    // here (a tainted canvas, an OOM on getImageData) left the
                    // promise unresolved forever AND the canvas unreleased —
                    // resolve()/reject() were never reached to know it happened.
